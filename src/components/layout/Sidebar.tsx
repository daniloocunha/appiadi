...
export function Sidebar({ pendingCount = 0, pendingRegistrationsCount = 0 }: { pendingCount?: number; pendingRegistrationsCount?: number }) {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-56 min-h-screen bg-blue-900 text-white shrink-0">
      {/* Logo */}
      ...
      <nav className="flex-1 py-4 px-2 flex flex-col gap-0.5">
        {navItems.map((item) => (
          ...
          >
            ...
            <ChevronRight size={14} className="text-blue-400 opacity-0 group-hover:opacity-100" />
          </NavLink>
        ))}
      </nav>
      ...
      <div className="px-3 py-3 border-t border-blue-800">
        ...
        <button
          ...
          className="flex items-center gap-2 w-full px-2 py-1.5 text-xs text-blue-300 hover:text-white hover:bg-white/10 rounded-md transition-colors"
        >
          <LogOut size={13} />
          Sair
        </button>
